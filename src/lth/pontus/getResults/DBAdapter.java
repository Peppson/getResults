...
public class DBAdapter {
	...
	public static final int COL_REP = 3;
	public static final int COL_WEIGHT = 4;
	public static final int COL_DATES = 5;

	public static final String[] ALL_KEYS = new String[] { KEY_ROWID, KEY_NAME,
			KEY_SETS, KEY_REP, KEY_WEIGHT, KEY_DATES };
	...
	private static final String DATABASE_CREATE_SQL = "create table "
			...
			// have NO comma!!
			+ KEY_NAME + " text not null, " + KEY_SETS + " integer not null, "
			+ KEY_REP + " integer not null, " + KEY_WEIGHT
			+ " integer not null, " + KEY_DATES + " string not null"
	...
	public long insertRow(String name, int sets, int rep, int weight,
			String date1) {
		/*
		 * CHANGE 3:
		 */
		...
		initialValues.put(KEY_DATES, date1);
	...
	public Cursor getRowByDate(String currentDate) {
		String dateInput = "date('" + currentDate + "')";
		String where = KEY_DATES + "=" + dateInput;
		Cursor c = db.query(true, DATABASE_TABLE, ALL_KEYS, where, null, null,
				null, null, null);
		if (c != null) {
			c.moveToFirst();
	...
	public Cursor getRowByExercise(String exercise) {
		String where = KEY_NAME + "=" + "'" + exercise + "'";
		Cursor c = db.query(true, DATABASE_TABLE, ALL_KEYS, where, null, null,
				null, null, null);
		if (c != null) {
			c.moveToFirst();
	...
	public Cursor getRowByset(int currentSet) {
		String where = KEY_SETS + "=" + currentSet;
		Cursor c = db.query(true, DATABASE_TABLE, ALL_KEYS, where, null, null,
				null, null, null);
		if (c != null) {
			c.moveToFirst();
	...
	public boolean updateRow(long rowId, String name, int sets, int rep,
			int weight, String date1) {
		String where = KEY_ROWID + "=" + rowId;
		...
		newValues.put(KEY_DATES, date1);

		// Insert it into the database.
		return db.update(DATABASE_TABLE, newValues, where, null) != 0;
	}

	public boolean updateRowNoDate(long rowId, String name, int sets, int rep,
			int weight) {
		String where = KEY_ROWID + "=" + rowId;
	...
	public Cursor getExerciseAboveId(long Id, String exerciseName) {
		String where = KEY_ROWID + ">" + Id + " AND " + KEY_NAME + "='"
				+ exerciseName + "'";
		Cursor c = db.query(true, DATABASE_TABLE, ALL_ROWIDS, where, null,
				null, null, null, null);
		if (c != null) {
			c.moveToFirst();
		}
